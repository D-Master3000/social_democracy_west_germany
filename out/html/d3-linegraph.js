...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['spd', 'fdp', 'cdu', 'npd', 'other'];
    }
    if (!partyColors) {
        partyColors = {'spd': '#E3000F', 'fdp': '#FFCC00', 'cdu': '#000', 'npd': '#954B00', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'spd': 'SPD', 'fdp': 'FDP', 'cdu': 'CDU + CSU', 'npd': 'NPD', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
